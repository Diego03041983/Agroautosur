...
export function LeadNoteForm({ leadId }: { leadId: string }) {
  ...
  return (
    <div className="grid gap-3 rounded-lg border border-[#152018]/10 bg-white p-4 shadow-sm">
      <div>
        <p className="text-[11px] font-black uppercase tracking-[0.16em] text-[#1f6b43]">Nota interna</p>
        <h2 className="mt-1 text-base font-black">Agregar seguimiento</h2>
      </div>
      <textarea
        value={note}
        ...
        className="aas-admin-field min-h-24 resize-y"
      />
      <button
        type="button"
        disabled={status === "loading" || note.trim().length < 3}
        onClick={submit}
        className="inline-flex h-9 items-center justify-center gap-2 rounded-md bg-[#183d2a] px-3 text-xs font-black text-white disabled:opacity-50"
      >
        Guardar nota <Send size={14} />
      </button>
